	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'stage':
				...
				case 'auditory':
				{
						defaultCamZoom = 0.7;
						curStage = 'auditory';
						var sky:FlxSprite = new FlxSprite(-2200, -1100).loadGraphic(Paths.image('bg/sky'));
						sky.setGraphicSize(Std.int(sky.width * 1.5));
						sky.setGraphicSize(Std.int(sky.height * 1.5));
						sky.antialiasing = true;
						sky.scrollFactor.set(0.9, 0.9);
						sky.active = false;
						add(sky);
	
						var grass:FlxSprite = new FlxSprite(-650, 730).loadGraphic(Paths.image('bg/grass'));
						grass.setGraphicSize(Std.int(grass.width * 9.8));
						grass.setGraphicSize(Std.int(grass.height * 9));
						grass.updateHitbox();
		...
		switch (SONG.player2)
		{
			...
			case 'tommy':
				dad.x -= 500;
				dad.y += 150;
				camPos.set(dad.getGraphicMidpoint().x + 500, dad.getGraphicMidpoint().y + -200);
			case 'zero':
				dad.x -= 60;
				dad.y += 250;
				camPos.set(dad.getGraphicMidpoint().x + 700, dad.getGraphicMidpoint().y + -400);
		}
